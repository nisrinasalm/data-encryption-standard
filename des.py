...
def des(text, rkb):
    ...
    for i in range(16):
        ...
        for j in range(8):
            ...
            col = bin_to_dec(int(xor_x[j * 6 + 1:j * 6 + 5]))
            val = sbox[j][row][col]
...
key = input("Enter the key (in hexadecimal, 16 characters): ")

rkb = key_generation(key)

cipher_text = des(pt, rkb)
print("Encrypted Text:", bin_to_hex(cipher_text))

plain_text = des(cipher_text, rkb[::-1])
print("Decrypted Text:", bin_to_hex(plain_text))
